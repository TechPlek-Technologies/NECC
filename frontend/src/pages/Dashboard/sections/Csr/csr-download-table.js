...
export const CsrDownloadTable = (props) => {
  const {
    count = 0,
    items = [],
    onPageChange = () => {},
  ...
  const handleUpdate = async () => {
    try {
      ...
      const formData = {
        ...
        pdfFileName: editedOffice.pdfFileName,
      };

      const response = await axios.put(
        `${domain}/csrpdf/${editedOffice.id}`,
        formData,
        {
          headers: {
  ...
  const handleDelete = async(id) => {
    try {
      const response = await axios.delete(`${domain}/csrpdf/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={12}>
              <TextField
                label="FileName"
                name="pdfFileName"
                value={editedOffice.pdfFileName}
                onChange={handleInputChange}
                fullWidth
              />
            </Grid>
